...
- **后端**：
  ...
  - 功能模块：
    ...
    - 文件服务：通过对象存储（MinIO/S3 兼容）处理背景图、蓝图上传，提供预签名直传、元数据管理与缩略图生成；对外访问统一经由 Web 网关 `/storage/` 子路径代理到对象存储，隐藏内部地址，后续可无缝接入 CDN。
...
3. **背景图/蓝图管理流程**：用户上传 → 前端请求对象存储预签名 URL → 浏览器直传至 MinIO/S3 → 上传完成后回传 `fileId`/`objectKey` 并写入布局版本元数据（保留尺寸与偏移）→ 前端根据后端返回的 `/storage/` 代理地址渲染蓝图 → 用户在蓝图模式下调整缩放/透明度/位置（独立于视口缩放）。
